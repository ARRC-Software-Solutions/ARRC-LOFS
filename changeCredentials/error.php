
<title>Change Credentials</title>
<link href="//db.onlinewebfonts.com/c/a4e256ed67403c6ad5d43937ed48a77b?family=Core+Sans+N+W01+35+Light" rel="stylesheet" type="text/css"/>

<link rel="stylesheet" href="form.css" type="text/css">
<div class="body-content">
  <div class="module">
  <center>
    <h1><b> Change Credentials </b></h1>

    <form class="form" action="index.php" method="POST" enctype="multipart/form-data" autocomplete="off">

    <div class="alert alert-error"> There was an error updating your account. Please check your credentials. </div>
      <input type="submit" value="Back" name="back" class="btn btn-block btn-primary" /><br><br>

      </center>

    <!-- <button class="btn btn-block1 btn-primary"  onclick="window.location.href = '//localhost/Myprojects/ARCprojects/guard_registration/index.php';">Create new account</button> -->
      

 
  </div>
</div>

